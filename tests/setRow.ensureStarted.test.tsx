...
const setSetStartTime = vi.fn();
vi.mock('@/store/workoutStore', () => ({
  useWorkoutStore: () => ({ startSet, setSetStartTime, setSetEndTime: () => {} }),
}));

describe('SetRow ensureStarted', () => {
  beforeEach(() => {
    startSet.mockReset();
    setSetStartTime.mockReset();
  });
  ...
  test('uses startSet when REST_FREEZE_ON_START enabled', () => {
    setFlagOverride('REST_FREEZE_ON_START', true);
    const { getByPlaceholderText } = renderRow();
    const input = getByPlaceholderText('Weight');
    fireEvent.focus(input);
    fireEvent.focus(input);
    expect(startSet).toHaveBeenCalledTimes(1);
    expect(setSetStartTime).not.toHaveBeenCalled();
  });

  test('uses setSetStartTime when REST_FREEZE_ON_START disabled', () => {
    ...
    expect(setSetStartTime).toHaveBeenCalledTimes(1);
  });
});
