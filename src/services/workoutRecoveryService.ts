  ...
  if (sets && sets.length > 0) {
    const { error: insertError } = await supabase
      ...
      .insert(sets.map(set => ({
        ...
        tempo: (set as any).tempo ?? null,
        range_of_motion: (set as any).range_of_motion ?? null,
        added_weight: (set as any).added_weight ?? null,
        assistance_used: (set as any).assistance_used ?? null,
        notes: (set as any).notes ?? null,
