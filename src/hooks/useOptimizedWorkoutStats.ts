...
// Optimized hook with parallel data loading and aggressive memoization
export function useOptimizedWorkoutStats() {
  const { user } = useAuth();
  ...
  // Optimized data fetching with aggressive memoization
  const fetchWorkoutData = useCallback(async () => {
    ...
    try {
      // Parallel data fetching - fetch workouts and user profile simultaneously
      const [workoutResponse, profileResponse] = await Promise.all([
        supabase
      ...
      const sessions = workoutResponse.data || [];
      setWorkouts(sessions);

      // Process stats with memoized calculations
      const processedStats = processWorkoutStats(sessions);
      setStats(processedStats);
      
    } catch (err) {
      console.error("[useOptimizedWorkoutStats] fetch error:", err);
    } finally {
      setLoading(false);
    }
  }, [user, queryDates.from, queryDates.to]);

  // Memoized stats processing function
  const processWorkoutStats = useCallback((sessions: any[]): WorkoutStats => {
      ...
      // Optimized counters with single pass
